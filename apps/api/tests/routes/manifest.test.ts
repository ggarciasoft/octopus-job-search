...
describe('route manifest', () => {
  ...
  it('defers exactly the M6 surface and nothing else', () => {
    // M1 and M2 landed: profile, imports, preferences, provider settings,
    // sources, scans and jobs are all registered. Only the three M6 routes
    // that need an email service remain. This list is pinned on purpose:
    // registering a route must delete its entry here, and deferring a new
    // one must add it, explicitly.
    expect(Object.keys(DEFERRED_OPERATIONS).sort()).toEqual(
      ['confirmPasswordReset', 'register', 'requestPasswordReset'].sort(),
    );
  });

  it('registers the whole M2 surface', () => {
    const registered = new Set(registeredOperations());
