...
| PR14 | Export/delete user data; backup/restore local installation       | **Export tested** | `POST /workspace/export` writes a real ZIP — versioned JSON plus the files — with a manifest that names what was deliberately left out, and the Privacy settings tab downloads it with its checksum. The deletion ledger (`0007_privacy`) records deletions and survives the workspace it names, and `scripts/restore.sh`/`.ps1` now save, merge and reapply it instead of refusing to. **Workspace deletion (`DELETE /workspace`, AT26) is not built**; the Privacy tab says so and offers no button. AT25 is not claimed: it needs a populated installation, not a test.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
