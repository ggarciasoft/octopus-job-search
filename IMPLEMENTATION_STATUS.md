...
| Verified working today              | Toolchain; `pnpm typecheck`, `pnpm lint`, `pnpm format:check`, `pnpm contracts:check`; every test suite (contracts 206, api 724, web 232, ui 7, worker 590, fill-planner 87, extension 105); two M4 workflows on the Compose stack — one full sandbox fill with a real Chromium, and one prepared to the approval gate against a live Greenhouse posting; the fixture corpus (46/46); all three images build; `docker compose up` from a `setup.sh`-generated `.env`; `scripts/smoke.sh` through the nginx proxy on `127.0.0.1:3000`; data persistence across `docker compose down`/`up`; `scripts/backup.sh` (gpg and `--no-encrypt`), `scripts/restore.sh` into a separate installation and over the source, and `scripts/migrate.sh` (AT25); the same run repeated under Windows PowerShell 5.1 with `smoke.ps1`, `backup.ps1` (gpg and `-NoEncrypt`), `restore.ps1` and `migrate.ps1` (2026-09-24); workspace deletion on a throwaway stack, and a restore of a pre-deletion backup that re-deleted it (AT26); all eleven M1–M3 screens rendered by a real Chromium against the live stack, in English and Spanish |
| **Never executed**                  | macOS/Linux hosts. (The fresh install on 2026-09-24 started with `--no-build`, because builds here need the build-CA workaround; the plain build runs in CI.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| PR09 | Autofill tested forms locally; final submit manual               | M4        | **Tested**      | Device pairing, `POST /applications/:id/fill`, the `fill_local` contract, a deterministic planner, the `greenhouse/v1` and `lever/v1` adapters, a persistent Chromium the runner owns, and the **Fill assistant panel**. Browser tests drive a real Chromium against a synthetic Greenhouse-shaped page (14) and a synthetic Lever-shaped one (22). It has **never run against a live board** and no form has been filled for a real application. Final submit is manual by construction: no code path clicks one, and the panel says so in every state.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| PR12 | Hosted app plus extension, accounts, quotas, isolation           | M5–M6     | **In progress** | Two M5 slices are landed and tested. **Scoped fill sessions** (migration `0011`, the `/fill-sessions` contract, four routes, 27 API cases): a session binds one approved packet to one device, one tab origin, one packet hash and one nonce, for ten minutes, spent once. **The extension** (`apps/extension`, Manifest V3): a service worker that holds the device token, a content script that holds no credential and decides nothing, a Greenhouse reader ported from the runner's own DOM snippets, and a popup. It builds into a loadable unpacked extension and has 39 tests. The planner both clients share is `packages/fill-planner`, a port of the runner's `forms.py` and `plan.py` pinned to it by golden vectors asserted from both sides (84 TypeScript cases, 84 Python). Loaded in a real Chrome on 2026-09-22 and **filled a form, CV included**. It now **pairs from a code** and **fills from a list**: the popup redeems a code minted in Settings → Devices, and `GET /fill-targets` (device-only, 9 API cases) lists the approved applications a session would be granted for. Nothing is pasted any more. Verified in a real Chrome, 19 of 19. A Lever reader exists in both clients since 2026-09-23 (see "M5: the Lever adapter"). Accounts, quotas and hosted isolation are M6.                                             |
...
| M5  | Open-source distribution and extension | **In progress** | A fresh user installs without developer intervention and fills supported pages  | The repository documentation half (README, CONTRIBUTING, SECURITY, NOTICE, issue templates, license) landed early with M0. Two slices since 2026-09-22: **scoped fill sessions** and **the extension itself**, with the shared planner and its cross-client parity fixtures between them — see PR12 and the three "M5" sections under "Verified commands". AT23 and AT24 both pass, the latter performed in a real Chrome against the page the extension had just filled — which completes the acceptance suite at 28 of 28. Pairing from a code and a popup that lists approved applications landed later on 2026-09-22 (see "M5: pairing from a code"). Install documentation landed the same day as `docs/INSTALL.md`: one path from a fresh clone to a filled form, with every step marked verified, partly verified or not. The settings file (export and import of preferences, CV template choice and boards) landed the same day. See "M5: the settings file". The **Lever fill adapter** landed on 2026-09-23 in both clients, against a synthetic page only. See "M5: the Lever adapter". Still to come: the permission prompt exercised by a person, and a live board.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
- Greenhouse and Lever discovery work and are verified. Their fill adapters,
  `greenhouse/v1` and `lever/v1`, are tested against synthetic fixtures only,
  and neither has filled a live board. LinkedIn, Workday and generic company
  career pages have no adapter. The support matrix in `README.md` says so, and
  no source may be shown as a working integration until it passes against
  fixtures (invariant 10).
...
- **No live form has been filled.** The paired desktop runner and the
  extension both fill synthetic Greenhouse and Lever pages and stop before
  submit. Neither has filled a real employer's form. The owner submits.
...
- AT25 ran the gpg and plaintext paths on Windows under Git Bash. The age
  path and the PowerShell scripts ran on 2026-09-24. macOS and Linux hosts have
  never run.
...
- The PowerShell scripts ran end to end under Windows PowerShell 5.1 and
  PowerShell 7 on 2026-09-24, and `restore.ps1` now finishes on both. See "The
  PowerShell scripts, run for the first time" and "The dev loop, age, PowerShell
  7 and a fresh install".
...
- The `ollama/ollama` image and CI's `postgres` service container are both
  pinned by digest since 2026-09-24, each checked against Docker Hub itself. See
...
### The dev loop, age, PowerShell 7 and a fresh install (verified 2026-09-24)

Everything that was still on "Explicitly NOT verified" for the scripts, run on
throwaway Compose projects (`jg-dev`, `jg-ps7`, `jg-ps7b`, `jg-fresh`), with
PowerShell 7.6.6 and age v1.3.1 installed for the purpose. Nine defects, all
fixed.

**The dev loop (`dev.sh`, `dev.ps1`) had never started.**

| Defect                                                                                                                                                               | Fix                                                                                                                                    |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| The database's host port was fixed at 5432. Another PostgreSQL held it here, as it often will on a contributor's machine. The message said "did not become healthy". | `JG_DEV_DB_PORT` moves it, `DATABASE_URL` is rewritten to match, and the message names the likely cause.                               |
| The host API and worker were given no `.env`, so they lacked the secrets they refuse to start without.                                                               | `.env` is read as data (never executed) into their environment, as Compose's `env_file` does. A variable set in your shell wins.       |
| Once running, the host API listened on every interface, including the LAN (`192.168.86.86:8080`). `HOST` defaults to `0.0.0.0` for the container.                    | The dev loop sets `HOST=127.0.0.1`. `DEV_HOST` changes it; an inherited `HOST` does not, because zsh sets one to the machine name.     |
| `dev.ps1` started the extensionless `pnpm` shim: "not a valid Win32 application".                                                                                    | It resolves `pnpm.cmd`. On shutdown it stops each process tree (`taskkill /T`), so the node servers under `cmd.exe` do not outlive it. |
| `dev.sh` fell from `wait -n` to a plain `wait` whenever the child that exited had failed, and always under dash. A crashed worker left the API and web running.      | It polls the children with `kill -0`.                                                                                                  |
| The documented dev-loop smoke command could only get a 403: the API trusts the Vite origin, and Vite does not proxy the health endpoints.                            | `smoke.sh --origin` and `smoke.ps1 -Origin` send a different Origin from the URL called.                                               |
| `.dev-files/`, which holds uploaded CVs in the dev loop, was not gitignored, though `dev.sh` said it was.                                                            | Ignored, with the temporary Compose override.                                                                                          |

Both loops then ran: 11 migrations from the host, the API on `127.0.0.1:8080`
only, Vite on 5173, and the host worker reporting 6 capabilities and 6
handlers. `smoke.ps1 -BaseUrl http://127.0.0.1:8080 -Origin
http://localhost:5173` and the `smoke.sh` equivalent passed. Killing the
worker's Python process made each script stop the rest, with ports 8080 and
5173 free and no process left. Before the `dev.sh` fix, the same kill left it
waiting with both servers up.

**age and PowerShell 7.** On `jg-ps7` under `pwsh`: `smoke.ps1` passed, then
`backup.ps1 -AgeRecipient` wrote a 173,872-byte `.tar.gz.age`. AT26's tool then
deleted the workspace (4 stored objects erased, every way back in refused, one
`workspace` row in the ledger). `restore.ps1` of the pre-deletion backup over it
unpacked 4 files, applied 1 ledger path and left **0 files**, the directory
included, and no workspace, user or resume rows. Login answered 401 and setup
was open again. This is the file pruning with real paths as part of a whole
script, which until now had run only on its own. The same backup restored by
Windows PowerShell 5.1 into the fresh `jg-ps7b` compared **6 of 6** with the
snapshot taken before it. `backup.sh --age-recipient` of `jg-ps7b` (which still
had the workspace), restored by `restore.sh` over `jg-ps7`, left 0 files and 0
workspaces there too.

Two defects on that path:

- **`restore.sh` could not verify any backup made by `backup.ps1`.**
  `WriteAllLines` wrote `SHA256SUMS` with CRLF, and `sha256sum` looked for
  `database.dump\r`, calling the archive corrupt. The comment on that line
  promised the opposite. `backup.ps1` now writes LF (0 CR bytes in a new
  backup), and `restore.sh` strips carriage returns from the list first, so the
  backups already made restore: the CRLF one from `jg-ps7` did. A byte appended
  to `manifest.json` is still refused.
- **Without `JG_AGE_IDENTITY`, restoring a `.age` backup failed as gzip and tar
  errors.** `restore.sh` said it would prompt, which age cannot do, and passed
  it an empty argument. Both restore scripts now stop up front and say what to
  set, and the variable is documented in their help and the runbook.

**A fresh install, following `docs/INSTALL.md`.** A clone of `main`, under
Windows PowerShell 5.1: `cp .env.example .env`, then `setup.ps1`, as the guide
says. **Setup refused**: both setup scripts refused any existing `.env`, so the
guide's first two commands had never worked together, though marked ✅. Now an
`.env` without an `ENCRYPTION_KEY` has its empty secrets filled, and one with a
key is still refused. Both scripts were checked both ways. The install then
came up healthy, and `smoke.ps1` completed its one-time setup with the new
token and passed. Two deviations from the guide: ports 3500/3501, because the
owner's stack holds 3000, and `--no-build`.

**One incident, on the owner's stack.** `docker-compose.yml` sets
`name: job-getter`, so the clone's first `up` joined the owner's project and
recreated the owner's api, web and worker containers from the clone's `.env`,
for about a minute. The owner's database container was not recreated. The
owner's stack was then recreated from its own `.env`. A check of every table
with an `updated_at` found one write in that window: the owner's own worker
re-registering. `docs/INSTALL.md` now says a second copy on one machine needs
its own `COMPOSE_PROJECT_NAME`. The `name:` stays: the owner's volume names
derive from it.

### The PowerShell scripts, run for the first time (verified 2026-09-24)

Windows PowerShell 5.1.26100 (`powershell.exe`; `pwsh` is not installed),
against two throwaway Compose projects, `jg-ps51` and `jg-ps51b`, from the
current images. `docker compose config` showed every volume and network named
for its own project before anything ran. The owner's stack was not touched. The
run repeated AT25 with the PowerShell scripts, using AT25's own filler
(`.local/at25/at25.py`, copied) and its throwaway gpg key.

**Three things stopped the scripts, all now fixed.**

1. **`smoke.ps1` sent a body on every GET.** Its `[string]$Body = $null`
   parameter becomes `''`, so the `$null -ne $Body` test was always true.
   Windows PowerShell 5.1 refuses a GET with a body ("Cannot send a content-body
   with this verb-type"). The readiness probe therefore never succeeded, and the
   script timed out after 90 s against a healthy stack. PowerShell 7 accepts a
   body on a GET, which is how this went unnoticed. It now tests
   `$PSBoundParameters.ContainsKey('Body')`.
2. **Redirected stderr killed `backup.ps1` and `restore.ps1`.** Under
   `$ErrorActionPreference = 'Stop'`, 5.1 turns each line a native program
   writes to a redirected stderr into a terminating error. `backup.ps1` died at
   `docker compose up -d --wait db`, with docker's own progress line
   (` Container jg-ps51-db-1 Running`) as the error message, on a command that
   succeeded. `restore.ps1` died at the same point, as AT26 recorded. A new
   `Invoke-JGNative` in `lib.ps1` runs a native command with stderr discarded
   and the preference relaxed for that call only. `$LASTEXITCODE` is kept, and
   every exit-code check still runs. All 21 `2>$null` and `*> $null` call sites
   in `backup.ps1`, `restore.ps1`, `migrate.ps1` and `lib.ps1` now go through
   it.
3. **Every document said `pwsh -File scripts/...`** (32 places, docs and script
   help). A stock Windows 11 has no `pwsh`, and `powershell -File` is refused
   there: running it with `-ExecutionPolicy Restricted`, the policy a stock
   client resolves to, printed "running scripts is disabled on this system".
   This machine never showed that, because its CurrentUser policy is
   `Unrestricted`. Every mention is now `powershell -ExecutionPolicy Bypass
-File ...`, which works under 5.1 and 7 and changes no machine setting.
   `scripts/README.md` says why.

**The run, in order:**

| Step                                                                                                                             | Result                                                                                                                                                                                                                                                         |
| -------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `smoke.ps1` on a fresh `jg-ps51` (bootstrap open)                                                                                | `SMOKE TEST PASSED`, exit 0. Setup completed, `noop_echo` echoed by `container-worker-1` (CPython 3.12.14), and the replay under the same Idempotency-Key returned the same task.                                                                              |
| `smoke.ps1` again (bootstrap closed)                                                                                             | Logged in instead, and passed, exit 0.                                                                                                                                                                                                                         |
| `at25.py populate`, then `snapshot` → `before.json`                                                                              | 2 jobs, 2 applications, 3 answers, 2 resumes, 4 files, 6 profile facts, 7 events.                                                                                                                                                                              |
| `backup.ps1 -GpgRecipient at25@example.invalid`, with api and worker running                                                     | Exit 0. `database.dump` of 141,416 bytes, `files.tar.gz` with 4 files, the manifest and `SHA256SUMS`, packed by the system `tar.exe` (bsdtar 3.8.8) into a 174,654-byte `.tar.gz.gpg`. The plaintext staging directory was removed.                            |
| `at25.py delete-answer ... notice_period` on `jg-ps51`                                                                           | 204. The ledger holds one `answer_bank` row. The backup predates it.                                                                                                                                                                                           |
| `restore.ps1 -From <.gpg> -DropExisting -Yes` into the fresh `jg-ps51b`, then `migrate.ps1`, then `docker compose up`            | All exit 0. Checksums matched, the `ENCRYPTION_KEY` fingerprint matched, 33 tables, 0 ledger rows to save or replay, 4 of 4 files. `migrate.ps1` chose Compose mode and reported 11 migrations up to date.                                                     |
| `snapshot` of `jg-ps51b`, `compare` with `before.json`                                                                           | **6 of 6.** Original CV and export byte-identical, both tailored formats download, the approval still bound to its content hash, no field differs.                                                                                                             |
| The same restore **over the source** `jg-ps51`, then `migrate.ps1` and `up`                                                      | Exit 0. It saved the target's 1 ledger row, merged it back after the restore and reapplied it.                                                                                                                                                                 |
| `snapshot` of `jg-ps51`, compared with `before.json`                                                                             | Answers 3 → 2. With `notice_period` taken out of `before.json` and both lists sorted by id, the snapshots are identical: **the deleted answer stayed deleted, and nothing else changed.** `compare` itself reports 5 of 6, because it diffs lists by position. |
| `backup.ps1 -NoEncrypt -Yes` of `jg-ps51b`, which still has `notice_period`, then `restore.ps1` of that `.tar.gz` over `jg-ps51` | Both exit 0. `jg-ps51` then holds `relocation` and `salary_expectation` only, while `jg-ps51b` still holds all three. The ledger held against a backup from another installation that carried the row.                                                         |
| `smoke.ps1` against the restored `jg-ps51`, as `owner@example.invalid`                                                           | `SMOKE TEST PASSED`, exit 0.                                                                                                                                                                                                                                   |

**The build path without TLS interception was already covered.** The status
listed it as never run, but CI's "Container build smoke" job runs
`docker compose build` on a GitHub runner, which does not intercept TLS. The
committed `docker-compose.yml` supplies no `build_ca` secret; only the
gitignored override on this machine does. Run 36051816085 on commit `569449b`
built web, api, worker and migrate that way, and every job passed.

**What this does not cover.** PowerShell 7. `-AgeRecipient`. `setup.ps1` and
`dev.ps1` against a live stack. The ledger here named no workspace or file, so
`restore.ps1`'s file pruning ran with an empty path list; that block has run
against real paths only on its own, in AT26. The gpg used was Git for Windows'
MSYS build, which needs `GNUPGHOME` in `/c/...` form: given `C:/...`, it looks
under the current directory. That is this machine's gpg, not the script. A
Gpg4win install reads Windows paths.

### M5: the Lever adapter (verified 2026-09-23)

`lever/v1` in the Python runner
(`services/worker/src/job_getter_worker/runner/adapters/lever.py`) and in the
extension (`apps/extension/src/adapters/lever.ts`), against a new synthetic
page, `fixtures/ats-pages/lever-application.html`, plus a `-changed` variant
with one more required question.

The fixture's shape came from reading one public Lever demo posting's apply page
(`jobs.lever.co/leverdemo/…/apply`) with a single GET. The page was not
committed, and nothing was filled or submitted. Running the new reader over
that saved page in Chromium gave 21 fields. The Greenhouse adapter did not
claim it, and every EEO and survey control came out `never_reuse`. Reading
the real page found three things a port of the Greenhouse reader would have
got wrong:

- **Greenhouse claimed Lever pages.** Lever's form is `form#application-form`,
  which is in the Greenhouse marker's selector. Greenhouse's marker now refuses
  a form built from Lever's `.application-question .application-label` blocks,
  and Greenhouse's `handles` refuses Lever's hosts. A test asserts every
  fixture is claimed by exactly one adapter, in Chromium and in jsdom.
- **The wrapping `<label>` is not the question.** On Lever it also holds the
  upload button's captions ("ATTACH RESUME/CV", "Analyzing resume...") and, on
  the EEO race question, a paragraph describing each option. The reader names a
  question by its `div.application-label`, an option by its
  `.application-answer-alternative`, and treats the U+2731 asterisk in
  `span.required` as required.
- **A disability-form signature labelled "Name".** The EEO section ends in a
  signature box labelled "Name" and "Date", and the survey asks "What is your
  age range?". The shared sensitivity list had nothing to catch either, so an
  answer stored under `name` would have signed a disability self-identification
  form. Every control in the EEO section or the survey is now labelled
  "Voluntary self-identification: …". `self-identif`, `pronoun` and
  `age range` join the `never_reuse` fragments in both planners, pinned by
  three new vectors.

The location field is an autocomplete, so it is reported as a widget for the
person to complete. Lever's pronouns group (a nameless "Custom" checkbox plus a
hidden text box that shares the group's name) is read as one question. The
confirmation reader is shared, and moved to `adapters/base.py` and
`adapters/confirmation.ts`. Lever's confirmation page has never been seen,
because seeing one means submitting an application. The extension's service
worker now names the adapter the content script reports, checked against
`KNOWN_ADAPTERS`, and names none for an unsupported page (it used to say
`greenhouse` either way).

```bash
cd services/worker
uv run pytest -o addopts="" -q                                # 590 passed
uv run pytest -o addopts="" -q tests/test_runner_lever.py \
  tests/test_planner_parity.py                                # 114 passed
uv run python ../../fixtures/fill-planner/generate.py         # 3 new sensitivity vectors
uv run python ../../fixtures/fill-planner/generate-page.py    # greenhouse-page.json unchanged;
                                                              # lever-page.json: v1:306a157c… (21), v1:7bb9366b… (22)
uv run python ../../fixtures/verify.py                        # 55/55
cd ../../apps/extension && npx vitest run                     # 105 passed
cd ../../packages/fill-planner && npx vitest run              # 87 passed
cd ../../apps/extension && npm run build                      # background.js 15.32 kB, content.js 10.65 kB
```

`fixtures/MANIFEST.sha256` had been missing `greenhouse-page.json` and
`greenhouse-confirmation.json`, so `verify.py` would have failed on them. Both
are now recorded, with the three new Lever files and the updated
`vectors.json`.

**In a real Chrome, 16 of 16.** `.local/e2e-m5-lever.py` is the Lever twin of
`e2e-m5.py`, with the same two sandbox shortcuts. It approved a packet for the
Lever fixture on the running Compose stack, and the built extension filled it.
The full name and email were typed. The work-authorization radio was exactly
"Yes", and the checkbox group exactly Python and Rust. The notice period was
"30 days", and the CV was attached to Lever's hidden file input
(`cv.pdf:30780`). The referral code, location, disability-form signature and
EEO gender were all left empty. The page never navigated. The application went
to `needs_input`, and its `fill_paused` event records `adapter: "lever"`,
`adapter_version: "v1"`. `e2e-m5.py` was re-run against Greenhouse after the
change and still passes 17 of 17.

Not verified: a Lever page on `jobs.lever.co` loaded by the runner or the
extension, a fill there, and a Lever confirmation page.
...
Customization level 2 in `08_UX_AND_CUSTOMIZATION.md`: "local YAML/JSON
import/export for reproducible settings", validated on import, unknown keys
rejected, and never a token, provider key or browser session. JSON only.
YAML would add a dependency and nothing the file needs.

- **What is in it.** The whole `Preferences` document, which already carries
  the CV template, the CV language, the resume mode and the prompt style
  suffix, plus the watched boards as `{connector, board_key, base_url,
enabled}`. **Deliberately not in it:** provider settings (a provider without
  its key would import half-configured), the answer bank (personal data, which
  the full workspace export on the Privacy tab covers), and board health or
  scan state. Every object in the contract
  (`packages/contracts/src/schemas/settings-file.ts`) is closed.
- **`GET /settings/export`** returns the file. Boards are sorted by connector
  and board key, so two exports of the same settings are the same file.
- **`POST /settings/import`** is all or nothing, in one transaction.
  Preferences are replaced against `expected_revision` (409 if they moved).
  Boards are **only added**: a board already present keeps its state, a board
  blocked by a site's 403/429 stays blocked, and a board the file omits is not
  deleted. A duplicated board or a `base_url` that is not a documented endpoint
  is refused with the board's position in the file
  (`settings.sources.1.base_url`). It writes one `settings.imported` audit
  event plus one `source.created` per new board, recording shape only.
- **Settings → Preferences → Settings file** has **Export settings** (a
  download named by the local date) and an import that reads the file, counts
  its boards, and changes nothing until the person confirms.

Tests: `apps/api/tests/settings-file.test.ts` has 22 cases: the export carries
no provider key, no provider, no answers and no health; a round trip into a
second workspace reproduces it; boards already present, blocked or omitted
are left alone; a second import adds nothing; and each refusal (unknown key at
the top, in a preference, in a board; another format or version; a non-board
connector; weights not summing to 100; a duplicate board; three kinds of bad
`base_url`; a stale revision; no CSRF token) leaves preferences **and** boards
untouched. `apps/web/tests/settings-file.test.tsx` has 5: the download holds
exactly what the API returned, nothing is sent before confirming or after
cancelling, non-JSON never reaches the API, and a refusal is shown field by
field.

**In a real Chromium:** `.local/e2e-m5-settings.py`, **17 of 17 checks**, run
twice. The owner's stack was only read. It exported through the Preferences
screen, and the downloaded file held exactly the owner's preferences and board
and none of `api_key`, `secret`, `token`, `health` or `answer`. That file was
imported through the screen into a throwaway `jg-scratch` installation, whose
worker was stopped so no board was actually scanned. The scratch stack's own
export then matched the owner's. A tampered copy (an unknown top-level key
plus a changed valid field) was refused naming the key, and nothing at all was
applied. The owner's revision was the same before and after. The scratch stack
was removed with its volumes. Screenshots: `.local/screenshots/m5-settings-*.png`.
The first run found a plural bug in the confirmation ("1 boards") and a file
named by the UTC date. Both are fixed.

### M5: pairing from a code, and a list instead of a pasted id (verified 2026-09-22)

Until now the extension could only be paired by exchanging a code by hand
and pasting the resulting token into the popup, and a fill needed the
application id and the packet hash pasted too. Neither is something a new
user could be expected to do. Both are gone.

- **Settings → Devices asks what is being paired.** It defaults to
  **Browser extension** (it used to hard-code `local_runner`), and the code
  panel tells you what to do with the code for that kind. For an extension,
  that means the address to type, which is this page's own origin.
- **The popup redeems the code itself** through the existing
  `POST /devices/exchange`. The token goes from the response straight into
  `chrome.storage.local`. The person never sees it, and the code is not kept.
  A token the API refuses with 401 (revoked, or thirty days old) is forgotten,
  and the popup asks to be paired again.
- **`GET /fill-targets`**, a new device-authenticated route, lists the
  approved applications this extension could open a fill session for now. The
  list and `POST /fill-sessions` use the same eligibility function
  (`fillRefusal`, extracted from `requireFillablePacket`) after the same
  reconciliation. So the list cannot offer something the session route would
  refuse, and an expired approval is withdrawn on the extension's read exactly
  as on the web app's. It returns pointers only: title, company, destination,
  hash and approval expiry. No answers, no CV.
- **The popup splits the list by the tab's origin.** Applications approved for
  that page appear under **For this page** with **Fill this page**. The rest
  offer **Open the application page** in a new tab, and only for an http(s)
  URL on the packet's own origin. Employer-sourced text is rendered with
  `textContent`.

Tests: `apps/api/tests/fill-sessions.test.ts` +9 (the list is a pointer and
not a packet; what it lists is what `POST /fill-sessions` accepts; it drops an
application once a fill is in progress, once its approval was withdrawn, or
once its approval expired, and in that case reconciles it to
`ready_for_review`; it omits an origin the device was not paired for; it
refuses a local runner (422), a session cookie (401) and a revoked device
(401)). `apps/extension/tests/pairing.test.ts` has 19 cases: the code goes out
once with no token and no cookies and is never stored; refusal, non-JSON and
unreachable addresses are each reported plainly; a 401 clears storage while an
outage does not; plus address and code normalisation, lookalike origins and
`javascript:` destinations. `apps/web/tests/applications.test.tsx` +2 (the
kind defaults to extension and the instructions name this origin; choosing a
local runner sends `local_runner`).

**In a real Chrome:** `.local/e2e-m5-pairing.py`, run against the Compose
stack after rebuilding the api and web images. **19 of 19 checks passed.**

| Step                                                   | Observed                                                                                                                                               |
| ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Settings → Devices, signed in in the browser           | "Browser extension" preselected; code shown; instructions name `http://127.0.0.1:3000`.                                                                |
| The popup, unpaired                                    | Shows the pairing form.                                                                                                                                |
| Address typed, code pasted with surrounding whitespace | Paired. Code field cleared. Storage holds a token and not the code. The web app lists the device as `paired`, named `job-getter-extension/<uuid>`.     |
| The list                                               | "Senior Platform Engineer — Northwind Robotics" under "Other approved applications" (the popup's own tab was active).                                  |
| **Open the application page**                          | Opened the fixture form in a new tab.                                                                                                                  |
| **Fill this page**, over the form tab                  | "Filled what the packet answers. Some questions still need you." `#first_name` reads `Ada`; the application left the list; the API says `needs_input`. |
| Device revoked in the API, popup reopened              | Pairing form, with "This browser's pairing was revoked or has expired. Pair it again." Token gone from storage.                                        |

Screenshots: `.local/screenshots/m5-pairing-web.png`,
`m5-pairing-popup-list.png` and `m5-pairing-filled.png`.

**Three sandbox shortcuts, named so nobody mistakes them for the product.**
The first two are `e2e-m5.py`'s and are unchanged: `apply_url` is seeded with
`psql`, and the run loads a copy of `dist` whose manifest declares the host
permissions. **The permission prompt therefore remains unexercised.** The
third is new. Automation opens the popup as a tab, and a tab's "active tab" is
itself, so for the one fill-from-the-list step an init script makes
`chrome.tabs.query({active, currentWindow})` answer with the form tab. That is
what Chrome does natively when the popup is opened over it. Everything else
is the real popup.

**Noticed on the way, not changed.** The owner's stack lists a dozen test
devices from earlier runs (`chrome-e2e`, `observe-live`, `sandbox`), all still
`paired` with live tokens. Revoking them is the owner's call.
...
loaded unpacked, and drives the whole flow. **17 of 17 checks passed.**
...
**The CV now attaches, and that closed the last gap in the fill itself.**
`GET /fill-sessions/:id/resume` serves the one file this packet was approved
with. The caller does not name it: the id in the path is the _session's_, and
the file is resolved from the packet that session was granted against, so
there is no parameter in which a page could ask for a different file — the
same property the runner's lease-scoped file endpoint has. The service worker
fetches the bytes and hands them to the content script, which never learns
where they came from or how to ask again. On the re-run, the form's file input
reads **`cv.pdf`, 30,780 bytes**, byte-for-byte the CV stored against the
packet. Seven API cases cover it: the nonce is required as well as the token,
an ended or expired session is refused, a revoked device is refused (AT23
extends to the CV), a second paired device gets a 404, and the bytes are served
`attachment` with `nosniff`.

A CV that fails to attach is now reported rather than dropped. The page might
refuse the assignment, and before this the report said nothing about the file
either way — so the person could believe a CV had been sent when none had.
It now becomes an unresolved `file_upload_blocked` question and the outcome
falls to `needs_input`, which is the documented download-and-attach fallback.

**What this still does not show.** No live employer's board: this is the
synthetic fixture, served from 127.0.0.1. Pairing, the application id and the
...
### Container image digests (verified 2026-09-20; Ollama and the Docker Hub check 2026-09-24)
...
| Image                          | Digest                      | Notes                                                                                                                                                                                                                                                                                               |
| ------------------------------ | --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `postgres:17-alpine`           | `sha256:f02121de…d867995`   | Container reports `postgres (PostgreSQL) 17.11`. Digest resolved via the ECR Public mirror. On 2026-09-24 the same digest also resolved on Docker Hub, where the `17-alpine` tag had moved on to `sha256:b0f9560a…0e552b24`. CI's service container now pins this digest too.                       |
...
| `ollama/ollama:0.34.0`         | `sha256:684d8674…58e0ba`    | Multi-arch index, resolved against Docker Hub on 2026-09-24 (not on the ECR mirror). Pulled by that digest (9.2 GB), started under `--profile local-ai`: healthy, `ollama --version` printed `0.34.0`, and the API container fetched `{"version":"0.34.0"}` from `http://ollama:11434/api/version`. |
...
### Explicitly NOT verified (as of 2026-09-24)
