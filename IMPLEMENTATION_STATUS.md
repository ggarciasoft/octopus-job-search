...
| Milestones complete                 | 4 of 8 (M0, M1, M2, M3); M4 in progress, 2 of 10 pilot workflows run — see the milestone table for what "complete" covers                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| PR04 | Import job URL or description; configure company boards          | M2        | **Tested**      | `POST /jobs/import` (URL or pasted text) and the source registry are live; verified on the Compose stack with a pasted-text import that resolved to a job carrying its salary excerpt. The web import panel (`apps/web/src/discovery/ImportPanel.tsx`, reached from Discover) is now built and covered by `apps/web/tests/jobImport.test.tsx` (8 tests): URL and pasted text are mutually exclusive by construction, a refused fetch states the refusal and offers the paste path without suggesting any circumvention, and a candidate list re-imports under a new idempotency key.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| PR11 | Configure providers, prompts, weights, limits, connectors        | M3–M5     | **In progress** | Four of the five are configurable and tested: providers through `GET`/`PUT /settings/providers` and the Provider screen (write-only, AES-256-GCM encrypted key), `match_weights` and `limits` through preferences, and connector settings through `PATCH /sources/:id`. Prompts are **not**: the only user-reachable control is `prompt_style_suffix`, and the prompt bodies themselves are not editable. Covered by `apps/api/tests/settings.test.ts` (88 cases) and `apps/web/tests/provider.test.tsx`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| PR12 | Hosted app plus extension, accounts, quotas, isolation           | M5–M6     | Not started     |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| PR13 | Discover company pages through an optional search API            | M6        | Not started     |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| PR14 | Export/delete user data; backup/restore local installation       | M4        | **In progress** | `POST /workspace/export` writes a real ZIP — versioned JSON plus the files — with a manifest that names what was deliberately left out, and the Privacy settings tab downloads it with its checksum. The deletion ledger (`0007_privacy`) records deletions and survives the workspace it names, and `scripts/restore.sh`/`.ps1` now save, merge and reapply it instead of refusing to. **Workspace deletion (`DELETE /workspace`, AT26) is not built**; the Privacy tab says so and offers no button. AT25 is not claimed: it needs a populated installation, not a test.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
- `scripts/restore.sh` **now reapplies the deletion ledger** (migration
  ...
  and runs `scripts/reapply-deletions.sql`. It still declines to describe its
  result as a verified restore, because AT25 — profile, files, hashes and
  history round-tripping — needs a populated installation to demonstrate and
  has not been run.
...
**Two other things the run surfaced, neither of them fixed here.**
...
1. Importing that board URL by hand produced the title
   ...
   That is the design working. What is missing is the correction afterwards:
   `PATCH /jobs/:id` accepts only `saved` and `status: closed`, so the only way
   to fix a title is to import the URL again with hints, which is what this run
   did.
