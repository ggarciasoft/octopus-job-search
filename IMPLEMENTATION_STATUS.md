...
**Last updated: 2026-09-20** · **M0 complete and verified.** Milestone in progress: **M1 (Profile)**
...
| ---- | --------------------------------------------------------------------------- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AT01 | Clean Compose start: health passes, schema created, owner setup works       | **Partly passed** | The application path is verified: `scripts/smoke.sh` passed end to end against a live stack (API from its compiled `dist/`, the Python worker, and PostgreSQL 17.11 in a container), with migrations applied from empty, readiness green, and one-time owner setup working and then refusing a still-valid token. **The Compose path itself is NOT verified: no image in `infra/` has ever been built and `docker compose up` has never been run.** Until that happens this row is not fully met. Also `apps/api/tests/db/schema.test.ts`, `tests/auth.test.ts`. |
...
| --- | -------------------------------------- | -------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| M0  | Foundation                             | **Complete**         | A runnable empty installation with an end-to-end fake task                      | Verified end to end: `scripts/smoke.sh` drives browser-origin -> API -> PostgreSQL queue -> Python worker -> stored result, with an Idempotency-Key replay returning the same task rather than duplicating work. Queue semantics exercised by hand over real HTTP too: a schema-violating result fails the task and stores nothing, and a stale lease token returns 409 with no domain effect. Remaining gap: the Compose images have never been built.                                                                                                                                          |
| M1  | Profile                                | **Largely complete** | Confirmed profile from fixtures; unknown/conflicting data visible               | Profile, imports, preferences and provider settings are implemented in the API and the worker, and verified live: a real PDF parsed to 19 draft facts, 18 confirmed into the profile with the withheld draft correctly discarded, US work authorization preserved as unknown. Provider secrets are write-only and AES-256-GCM encrypted; the provider probe refuses private, loopback, link-local and cloud-metadata destinations including after redirects. **Not done: the web UI for these screens is still the not-implemented placeholder**, so the flow is reachable only through the API. |
...
- **`scripts/smoke.sh` — the M0 exit criterion — now passes** against a live
  stack started by hand (compiled API, Python worker, PostgreSQL 17.11).
  Running it uncovered a real cross-platform defect in `scripts/lib.sh`:
  `random_b64` deleted only the newline from `openssl rand -base64` output, so
  on Windows every generated value kept a trailing carriage return. That CR is
  invisible in a terminal and reached every generated secret, including the
  `SETUP_TOKEN` a user is told to copy from their console, and it made the
  probe request malformed at the HTTP protocol level so it never reached a
  route. Fixed and verified over 100 draws.
- **The Compose path remains unverified.** No image in `infra/` has been built
  and `docker compose up` has never been run, so AT01 is only partly met and
  `smoke.ps1` is syntax-checked only.
