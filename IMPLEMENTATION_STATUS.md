...
| **Never executed**                  | `scripts/dev.*` (syntax-checked only); `backup.sh --age-recipient` and `backup.ps1 -AgeRecipient` (only the gpg and plaintext paths ran); any `.ps1` under PowerShell 7; `restore.ps1`'s file-pruning with a non-empty path list under the full script (it ran on its own under 5.1 in AT26); macOS/Linux hosts                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
- The PowerShell scripts ran end to end under Windows PowerShell 5.1 on
  2026-09-24, and `restore.ps1` now finishes there. PowerShell 7 has still never
  run them. See "The PowerShell scripts, run for the first time".
...
| `scripts/backup.sh --age-recipient`, `backup.ps1 -AgeRecipient` | Never run. Only the gpg and plaintext paths were exercised, by AT25 and by the 5.1 run on 2026-09-24.                                                                          |
| Any `.ps1` under PowerShell 7                                   | Never run. `pwsh` is not installed here; every PowerShell run so far was Windows PowerShell 5.1.                                                                               |
| `restore.ps1` pruning deleted files, as a whole script          | The 2026-09-24 run applied a ledger with no workspace or file entries (0 paths). The pruning block itself ran on its own under 5.1 in AT26, against real paths.                |
