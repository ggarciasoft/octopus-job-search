...
| Acceptance scenarios passing        | 27 of 28 (AT01–AT23, AT25–AT28); AT24 is partial — its API half is built and tested, its service-worker half needs an extension that does not exist yet                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Milestones complete                 | 4 of 8 (M0, M1, M2, M3); M4 in progress, 2 of 10 pilot workflows run; M5 started with its security core. 27 of 28 acceptance scenarios pass, every pilot-gate scenario among them — see the milestone table for what "complete" covers                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Verified working today              | Toolchain; `pnpm typecheck`, `pnpm lint`, `pnpm format:check`, `pnpm contracts:check`; every test suite (contracts 192, api 640, web 223, ui 7, worker 471); two M4 workflows on the Compose stack — one full sandbox fill with a real Chromium, and one prepared to the approval gate against a live Greenhouse posting; the fixture corpus (46/46); all three images build; `docker compose up` from a `setup.sh`-generated `.env`; `scripts/smoke.sh` through the nginx proxy on `127.0.0.1:3000`; data persistence across `docker compose down`/`up`; `scripts/backup.sh` (gpg and `--no-encrypt`), `scripts/restore.sh` into a separate installation and over the source, and `scripts/migrate.sh` (AT25); workspace deletion on a throwaway stack, and a restore of a pre-deletion backup that re-deleted it (AT26); all eleven M1–M3 screens rendered by a real Chromium against the live stack, in English and Spanish |
...
| PR12 | Hosted app plus extension, accounts, quotas, isolation           | M5–M6     | **In progress** | The first M5 slice is landed and tested: **scoped fill sessions**, the credential the browser extension will fill with. Migration `0011_fill_sessions`, the `/fill-sessions` contract and four routes that are the entire API surface a device token can reach. A session binds one approved packet to one device, one tab origin, one packet hash and one nonce, for ten minutes, spent once — 07_APPLICATION_AUTOMATION.md's list, in a table and four handlers. `apps/api/tests/fill-sessions.test.ts` (27 cases) covers it, including AT23 and the API half of AT24. These are also the first routes in the product authenticated by `auth: 'device'`: the mode existed in the contract and no route had ever used it, so `buildPreHandler` gained the branch that enforces it. **The extension itself does not exist.** `apps/extension/` is an empty directory; nothing has packaged an adapter, held a token in a service worker or filled a page. Accounts, quotas and hosted isolation are M6.                                                                                                                                                                                                                                                                                                                                                 |
...
| AT24 | Malicious page message cannot reach token/profile                           | **Partial** | The API half is built and tested (`apps/api/tests/fill-sessions.test.ts`), and it is three separate refusals because they would fail three different ways. **The token:** the `/fill-sessions` routes take `x-device-token` and nothing else, so a page riding the victim's session cookie gets 401 — the token lives in the extension's service worker, which page scripts cannot reach. **The profile:** the grant is asserted to carry no profile, no `password_hash`, no `token_hash` and no `secret_ciphertext`; it holds this packet's answers and a CV file id, never bytes. **The destination:** it comes from the approved packet, and a request that tries to supply one is a 400 from the closed contract rather than a quietly ignored field — ignoring it would look identical to honouring it. A path, query or fragment on the origin is stripped by `new URL().origin`. **What is missing is the other half of the scenario:** "Service worker verifies message sender/tab/origin; do not trust page-origin window messages as privileged commands." That code does not exist, because the extension does not exist. |
...
gate (all scenarios) is **not met**: 27 of the 28 pass, and AT24 needs the
extension's own message handling, which is not built.
...
| M5  | Open-source distribution and extension | **In progress** | A fresh user installs without developer intervention and fills supported pages  | The repository documentation half (README, CONTRIBUTING, SECURITY, NOTICE, issue templates, license) landed early with M0. Started 2026-09-22 with the security core rather than the visible part: **scoped fill sessions** (migration `0011`, the contract, four routes, 27 tests) — see PR12 and "M5: scoped fill sessions" under "Verified commands". AT23 now passes and AT24 is half done. Still to come, and it is most of the milestone: the extension itself (`apps/extension/` is an empty directory), the adapter logic packaged as TypeScript, template/settings export, install documentation and a second ATS adapter.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
