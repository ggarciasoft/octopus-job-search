...
- CI: `contracts`, `fixtures`, `hygiene`, `node-quality`, `node-test` and
  `worker` all pass locally. `compose-build` is verified locally **with** the
  `build_ca` secret; a GitHub runner has no interception, so the guards are
  no-ops there, but that has not been observed. The header of
  `.github/workflows/ci.yml` says exactly this.
