...
| AT24 | Malicious page message cannot reach token/profile                           | **Partial** | Both halves are now built and tested, and the reason this is not "Passed" is that no browser has ever loaded the extension. **API** (`apps/api/tests/fill-sessions.test.ts`): a page riding the victim's session cookie gets 401, because these routes take a device token that lives in a service worker page scripts cannot reach; the grant carries no profile and no secret, asserted on the serialised body; and the destination comes from the approved packet, so supplying one is a 400 from the closed contract rather than a quietly ignored field. **Extension** (`apps/extension/tests/messages.test.ts`, 9 cases): the extension id is not a secret, so `chrome.runtime.sendMessage` is reachable by any page that learns it — a caller with no tab, a different tab, a subframe, or the right tab on the wrong origin is refused, through the handler as well as the verifier. There is deliberately no `window` message listener anywhere in `src/`. **What is untested:** the extension has never run in Chrome, so nothing has established that a real hostile page meets these checks rather than some path around them. |
