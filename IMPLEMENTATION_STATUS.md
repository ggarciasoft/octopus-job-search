...
**Last updated: 2026-09-21** · **M0, M1, M2, M3 complete; M4 in progress** — the application backend and the paired local runner are landed and tested, including a real Chromium filling a synthetic Greenhouse form and stopping before submit. The M4 screens and export/backup are not built, and no adapter has met a live board. No browser has rendered the M2, M3 or M4 screens. Next: **the Application review, Fill assistant and Tracker screens**.
...
|                                     |                                                                                                                                                                                                                                                                                                                                                                                             |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Functional requirements implemented | 7 of 14 (PR01–PR07); PR08, PR09 and PR10 are tested with no screen yet                                                                                                                                                                                                                                                                                                                      |
...
| Verified working today              | Toolchain; `pnpm typecheck`, `pnpm lint`, `pnpm format:check`, `pnpm contracts:check`; every test suite (contracts 191, api 550, web 174, ui 7, worker 439); the fixture corpus (46/46); all three images build; `docker compose up` from a `setup.sh`-generated `.env`; `scripts/smoke.sh` through the nginx proxy on `127.0.0.1:3000`; data persistence across `docker compose down`/`up` |
...
| PR08 | Reviewable application packet and answer bank                    | **API**          | Contracts, migration `0005_applications`, `POST /applications`, `POST /applications/:id/packets`, `POST /applications/:id/approve` and the answer-bank routes are landed and tested (34 API cases plus 28 schema cases). A packet snapshots the profile and job revisions, the CV bytes and the destination resolved from the job's own provenance; approval is bound to a SHA-256 of that content and expires. The **Application review screen does not exist**, so nothing reviewable has been rendered to a person yet.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| PR09 | Autofill tested forms locally; final submit manual               | **Runner + API** | The paired local runner is landed: device pairing with single-use codes and 30-day scoped tokens, `POST /applications/:id/fill`, the `fill_local` contract, a deterministic planner, the `greenhouse/v1` adapter and a persistent Chromium the runner owns. 14 browser tests drive a real Chromium against a synthetic Greenhouse-shaped page. It has **never run against a live board**, no form has been filled for a real application, and the Fill assistant screen does not exist. Final submit is manual by construction: no code path clicks one.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| PR10 | Track outcome with evidence, history, duplicate prevention       | **API**          | `POST /applications/:id/outcome`, `GET /applications`, `GET /applications/:id/events`. Evidence is typed and a browser session cannot claim `adapter_observed`; the event log is append-only, enforced by a trigger rather than by convention. Duplicate prevention is two-layer: `UNIQUE (workspace_id, job_id)` makes two creates return one application, and other applications for possibly-duplicate jobs are reported, never merged. The **Tracker screen does not exist**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
- **No M4 screen exists.** Application review, Fill assistant and Tracker are
  API-only. Everything the M4 backend does today is reachable through HTTP and
  through tests, and through nothing a person can click.
